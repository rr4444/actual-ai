...
*(Note: Matched Amazon refund/income transactions processed by the uploader under the `amazonNoterWorkflow` are an exception. They will still be examined and classified even if `includeIncome` is disabled — see details under `amazonNoterWorkflow` below).*
...
**Amazon Refunds & `includeIncome` Disabled Mechanics:**
When `includeIncome` is disabled (the default setting) but `amazonNoterWorkflow` is enabled, the system handles Amazon refund (income) transactions through the following filter logic:
- **Raw Amazon Refunds (Unprocessed)**: If a transaction has a positive amount (income) and contains "Amazon" or "Amz" in the payee or notes, but does **not** contain uploader notes (like `#Amazon-Product-Name` or `#Amazon-Order-ID`), the system skips/filters it out. This prevents the LLM from incorrectly or prematurely categorizing the raw refund.
- **Processed Amazon Refunds (Tagged)**: Once the uploader matches the refund against your order/refund CSV files and writes the details (e.g., `#Amazon-Product-Name: Whole Earth Crunchy Peanut Butter`), the transaction now has the uploader notes. In this case, `actual-ai` recognizes that it has been processed. It allows the transaction through the filter (bypassing the `includeIncome` disabled skip) and categorizes it using the matched product name.
